        ...
        ScrollView(.vertical, showsIndicators: false) {
            ForEach(profiles) { profile in
                ForEach(profile.posts) { post in
                    RankingView(user: profile, image: profile.image, username: profile.id, post: post, profile: false, discover: false, displayHeader: $displayHeader, postExpanded: $postExpanded, feedExpanded: $feedExpanded)
